<?php
require_once 'connect.php';
$user_id = $_POST['user_id'];
$tournament_id = $_POST['tournament_id'];
$inform = mysqli_query($connect_user, "SELECT * FROM `users` WHERE `id` = '$user_id'");
$inform = mysqli_fetch_all($inform);
      foreach ($inform as $info){
              $id = $info[0];
              $login = $info[1];
              $pass = $info[2];
              $last_name = $info[3];
              $first_name = $info[4];
              $country = $info[5];
              $city = $info[6];
      }

$team_info = mysqli_query($connect_user, "SELECT * FROM `tables` WHERE `tournament_id` = '$tournament_id' ORDER BY `points` DESC");
$team_info = mysqli_fetch_all($team_info);
 ?>

<!DOCTYPE html>
<html lang="ru">
  <head>
    <meta charset="utf-8">
    <title>Создание команд</title>
    <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css"
     integrity="sha384-JcKb8q3iqJ61gNV9KGb8thSsNjpSL0n8PARn9HuZOnIxN0hoP+VmmDGMN5t9UJ0Z" crossorigin="anonymous">
     <style>
     .edit{
       width: 50%;
       position: absolute;
       overflow: auto;
       top: 15%;
       left: 25%;
       border: 3px solid black;
       background-color: white;
       padding-left: 15px;
       padding-right: 15px;
       border-radius: 20px;
     }
     .information{
       font: normal small-caps 30px/30px fantasy;
       background-color: white;
       height: 35px;
     }
     table, th, td{
       font: normal small-caps 50px/30px;
       border: 1px solid black;
       text-align: center;
     }
     </style>
  </head>
  <body background="../pictures/THW_RNL_Totale_f80f9_f_1920x1080.jpg">
    <div class="information">
      <div style="width: 50%; text-align: left; float: left;"> <?=$last_name?> <?=$first_name?> (<?=$country?>,<?=$city?>) </div>
    </div>
    <div class="edit">
    <h3 align="center">Редактирование данных</h3>
      <form action="edit_team_name_prossess.php" class="autorisation-registration-edit" method="post">
        <input type="hidden" class="form-control" name="tournament_id" value="<?=$tournament_id?>">
        <input type="hidden" class="form-control" name="start_id" value="<?=$team_info[0][0]?>">
        <input type="hidden" class="form-control" name="count" value="<?=count($team_info)?>">
        <?php
        foreach($team_info as $info){
        ?>
          <input type="hidden" class="form-control" name="id<?=$info[0]?>" id="id" value="<?=$info[0]?>">
          <input type="text" class="form-control" name="name_team_<?=$info[0]?>" id="id" value="<?=$info[2]?>" placeholder="Изменить название команды">
        <?php } ?>
        <button type="submit" class="btn btn-success" name="button">Изменить данные</button>
        <button type="reset" class="btn btn-success" name="button">Сбросить</button><br>
      </form>
    </div>
  </body>
</html>
